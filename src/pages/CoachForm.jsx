...
export default function CoachForm() {
  ...
  const handleSubmit = (e) => {
    ...
    console.log("Treinador:", form);
    alert("Treinador registado (console).");
  };

  return (
    <div>
      <h2 className="text-2xl font-bold mb-4">Registar Treinador</h2>
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white p-6 rounded shadow">
        ...
        <input name="licenca" placeholder="Licença" value={form.licenca} onChange={handleChange} className="p-2 border rounded" />
        <input name="anosExperiencia" type="number" placeholder="Anos de experiência" value={form.anosExperiencia} onChange={handleChange} className="p-2 border rounded" />
        <input name="clubesTreinados" placeholder="Clubes treinados" value={form.clubesTreinados} onChange={handleChange} className="p-2 border rounded" />
        ...
        <input name="especialidade" placeholder="Especialidade" value={form.especialidade} onChange={handleChange} className="p-2 border rounded" />
        ...
        <div className="md:col-span-2 border p-3 rounded bg-gray-50">
          ...
          {form.hasReadPolicies && (
            <div className="mt-2 text-sm text-gray-700 max-h-40 overflow-y-auto">
              ...
              <p>Os dados são usados para gestão de carreira e contactos institucionais. Consulte a política completa.</p>
            </div>
          )}
        </div>
